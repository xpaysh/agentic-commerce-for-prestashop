...
import { PrestaShopAdapter } from "./adapter";
...
function buildHandler() {
  ...
  const ps = new PrestaShopClient(config.prestashop);
  const adapter = new PrestaShopAdapter({ prestashop: ps, siteUrl: config.siteUrl });

  const exactRoutes: Record<string, RouteHandler> = {
    ...buildDiscoveryRoutes(config),
    "/healthz": buildHealthRoute(ps, VERSION),
...
async function main() {
  ...
  server.listen(config.port, config.host, () => {
    ...
    console.log(
      `agentic-commerce-for-prestashop v${VERSION} listening on http://${config.host}:${config.port}`,
    );
    // eslint-disable-next-line no-console
    console.log(`  merchant slug:       ${config.merchantSlug}`);
    // eslint-disable-next-line no-console
    console.log(`  prestashop base:        ${config.prestashop.baseUrl}`);
    // eslint-disable-next-line no-console
