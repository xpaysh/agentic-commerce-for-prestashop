...
import type { PrestaShopAdapter } from "../adapter";
import type {
  Cart,
  Order,
  Product,
...
export function buildUcpRouteTable(adapter: PrestaShopAdapter): RouteTable<RouteHandler> {
...
function catalogSearch(adapter: PrestaShopAdapter): RouteHandler {
...
function catalogLookup(adapter: PrestaShopAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
...
function createCart(adapter: PrestaShopAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
...
function getCart(adapter: PrestaShopAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
    const id = req.params?.id;
...
function updateCart(adapter: PrestaShopAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
    const id = req.params?.id;
    if (!id) return bad(400, "missing_id", "cart id required in path");
...
function checkout(adapter: PrestaShopAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
...
function getOrder(adapter: PrestaShopAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
    const id = req.params?.id;
