...
const sessions = createSessionStore({
  pluginId: process.env.ACP_SESSION_PLUGIN_ID || "agentic-commerce-for-prestashop",
});
...
export function buildAcpRouteTable(adapter: PrestaShopAdapter): RouteTable<RouteHandler> {
...
function createSession(adapter: PrestaShopAdapter): RouteHandler {
...
function updateSession(adapter: PrestaShopAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
...
function getSession(adapter: PrestaShopAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
    const id = req.params?.id;
    if (!id) return acpError(400, "invalid_request", "checkout_session_id required");
    const s = await sessions.get(id) as AcpSession | null;
...
function completeSession(adapter: PrestaShopAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
    const id = req.params?.id;
    if (!id) return acpError(400, "invalid_request", "checkout_session_id required");
    const s = await sessions.get(id) as AcpSession | null;
...
function getOrderRoute(adapter: PrestaShopAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
    const id = req.params?.id;
