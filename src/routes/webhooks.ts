/**
 * PrestaShop order webhooks → normalized OrderStateChanged events.
 *
 * PrestaShop has no native outbound webhooks. The companion PrestaShop
 * module (`xpayagenticcommerce`) hooks `actionOrderHistoryAddAfter`,
 * `actionValidateOrder`, `actionPaymentConfirmation`, and POSTs a normalized
 * payload to `${SITE_URL}/webhooks/prestashop` with shared secret in
 * `X-Xpay-Webhook-Secret`.
 *
 * Payload shape (defined by the bridge module):
 *   {
 *     event: "order.created" | "order.updated" | …,
 *     order: { id_order: "1042", reference: "ABCDEF", current_state: "5" }
 *   }
 */
...
export function buildWebhookRouteTable(): RouteTable<RouteHandler> {
  ...
  table.add("POST", "/webhooks/prestashop", buildPrestaShopWebhookRoute());
  return table;
}

export function buildPrestaShopWebhookRoute(): RouteHandler {
  return async (req): Promise<RouteResponse> => {
    ...
    let payload: { event?: string; order?: { id_order?: string; reference?: string } } & Record<string, unknown>;
    try {
    ...
    const orderId = payload.order?.id_order || payload.order?.reference;
    if (!orderId) return jsonError(400, "missing_order_id", "order.id_order or order.reference required");
